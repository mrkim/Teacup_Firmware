...
/** \file
	\brief Pull in appropriate acceleration algorithm, or use a default one if none selected
*/

#ifdef	ACCELERATION_REPRAP
	#include	"acceleration_reprap.c"

#elif defined ACCELERATION_RAMPING
	#include	"acceleration_ramping.c"

#elif defined ACCELERATION_TEMPORAL
	#include	"acceleration_temporal.c"

#else

	// data structure for no acceleration
	typedef struct {
		uint32_t	c;
	} ACCELERATION_DATA_NONE;

	// buffer for no acceleration data
	ACCELERATION_DATA_NONE acceleration_data_buffer[MOVEBUFFER_SIZE];

	// none selected, don't accelerate at all
	// this may make a good template
	void acceleration_create(uint8_t move) {
		uint32_t distance;
		DDA *dda = &movebuffer[move];
		ACCELERATION_DATA_NONE *aad = &accel_data_buffer[move];
	...
	uint32_t acceleration_start(uint8_t move) {
		...
		return accel_data_buffer[move].c;
	}
...
#endif /* no acceleration selected */
